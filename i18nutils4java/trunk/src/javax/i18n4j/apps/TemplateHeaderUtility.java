...
public class TemplateHeaderUtility {
    ...
    private static final Logger logger =
	    Logger.getLogger(TemplateHeaderUtility.class);
    private static final Translator translator =
	    Translator.getTranslator(TemplateHeaderUtility.class);
    ...
    private static void addHeaderToFile(File template, File about,
	    File file) {
	if (!template.exists()) {
	...
	RTAParser rta =
		new RTAParser(args, "Template Header Utility",
			TemplateHeaderUtility.class, translator
				.i18n("adds headers to source files."));
	rta.readStandardSwitches();
	String headerTemplate =
		rta.getFreeParameterString(translator
			.i18n("Header template file"));
	String aboutFile =
		rta.getFreeParameterString(translator.i18n("About file"));
	...
	for (File file : FileSearch.find(searchPattern)) {
	    logger.info("Processing file '" + file.getPath() + "'...");
	    addHeaderToFile(new File(headerTemplate), new File(aboutFile),
		    file);
	    logger.info("done.");
	}
    }
