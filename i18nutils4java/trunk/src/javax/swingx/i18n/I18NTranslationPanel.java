...
public class I18NTranslationPanel extends Panel {
	...
	locales.connect("changedSelection", this,
		"setLocale", Object.class);
	add(Label.addTo(new ScrollPane(locales), translator
		.i18n("Language"), Label.TOP),
		BorderLayout.NORTH);
	...
	reservoir.connect("valueChanged", this,
		"changeSource", Object.class);

	centerPanel.add(Label.addTo(new ScrollPane(
		reservoir), translator.i18n("Reservoir"),
		Label.TOP));
	source = new TextArea();
	...
	translation.connect("changeText", this,
		"changeTranslation", String.class);
	add(centerPanel, BorderLayout.CENTER);

	location = new TextArea();
	location.setEditable(false);
	centerPanel.add(Label.addTo(
		new ScrollPane(location), translator
			.i18n("Location(s)"), Label.TOP));

	add(new Label("Some I18NFile statistics"),
		BorderLayout.SOUTH);
	add(
		new Label(
			"<html>Some project<br/>statistics</html>"),
		BorderLayout.EAST);
    }
	...
	files =
		FileSearch.find(directory.getPath()
			+ "/res/**/*.i18n");
	...
	for (String source : sources) {
	    boolean translated =
		    translationsHash.getAvailableLanguages(
			    source).contains(
			    currentLocale.getLanguage());
	...
	LanguageSet languageSet =
		translationsHash.get(source.getText());
	if (languageSet == null) {
	    location.setText("");
	    return;
	}
	for (SourceLocation location : languageSet
		.getLocations()) {
	...
	if ((!translation.getText().isEmpty())
	    ...
	    if ((!oldSource.isEmpty())
		    && (!oldLanguage.isEmpty())) {
		translationsHash.set(oldSource,
			oldLanguage, translation.getText());
		changed = true;
		translationChanged = false;
	    }
	}
