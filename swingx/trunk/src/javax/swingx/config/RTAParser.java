 ...
 * implemented in a way that all needed command line switches like --help
 * and --version are implemented and documented automatically.
 * 
 * The object itself contains only methods for reading the command line
 * arguments, but forces to tell during read what the command line argument
 * is useful to. These information are collected in background and can be
 * shown afterwards in a help screen.
 * 
...
public class RTAParser {
    ...
    /**
     * This constant is used by reading non-stackable command line
     * arguments.
     */
    ...
    /**
     * getParameterString returns the string given for a special parameter.
     * This method also can only be used once per parameter. See
     * isSwitchSet for details.
     * 
     * @see RTAParser#isSwitchSet(String, String, boolean, String)
     * @param shortForm
     *            is a single character which is the short represenation of
     *            the parameter.
     * @param longForm
     *            is a string which represents the long form of the
     *            parameter.
     * @param description
     *            is a string describing the parameter. This is used for
     *            showUsage().
     * @return The parameter string is returned. If the returned length is
     *         zero, the parameter was not found!
     */
    public String getParameterString(String shortForm, String longForm,
	    String description) {
	parameterList
		.add(createHelpLine(shortForm, longForm, description));
	if (argv.length <= 0) {
	    return "";
	}
	String resultStr = "";
     ...
     * 
     * @return The return value is true if all parameters were checked out
     *         of the parameter array. Otherwise false is returned.
     */
    ...
    /**
     * This method returns the number of appearances of the parameter. This
     * is an internal method for private use and a more detail
     * documentation is therefore not needed.
     * 
     * @param index
     ...
     * @param stackable
     *            is a boolean which sets the possibility to use more the
     *            one short form behind a single '-'.
     * @return The number of appearances of the switch are returned. There
     *         is no difference between short and long form. Each occurence
     *         is counted.
     */
    private int isSwitchIncluded(int index, String shortForm,
	    String longForm, boolean stackable) {
	...
	if ((argv[index].equals(s)) || (argv[index].equals(l))) {
		...
		for (int i = 1; i < argv[index].length(); i++) {
			...
			argv[index] =
				argv[index].replaceFirst(shortForm, ".");
     ...
     * Attention(!): Parsing is not reproducible! Searching for a parameter
     * can only be done once. If another scan for this parameter is needed,
     * an initializeParser() is to be executed again, because the Parameter
     * which were read are overwritten in the object, so it is possible to
     * check, that all parameters were read and all parameters are used.
     * Check out the method printErrorMessage(). This is the method, which
     * checks for all parameter read and prints an error message, if there
     * are parameters left in the buffer.
     * 
     * @see RTAParser#getParameterString(String, String, String)
     * @param shortForm
     *            is a string which represents the short form of the
     *            parameter.
     * @param longForm
     *            is a string which represents the long form of the
     *            parameter.
     ...
     *            is a string describing the parameter. This is used for
     *            showUsage().
     * @return A boolean is returned as result. TRUE is returned in the
     *         case the parameter fits and FALSE if not.
     */
    ...
    /**
     * getFreeParameterString returns the first string which does not start
     * with '-'. Attention(!): This parameters have to have a fixed
     * position within the argument list, because they are not marked with
     * special characters!
     * 
     * @param description
     *            is a string describing the parameter. This is used for
     *            showUsage().
     * @return The parameter string is returned. If the returnlength is
     *         zero, the parameter was not found!
     */
     ...
     * 
     * @param line
     *            is a String to be added to the descriptions section of
     *            the help screen.
     */
     ...
     * localization (--locale). The first two are set directly in
     * APIConfig. The last is set with Translator.
     */
	...
	APIConfig.setVersionRequest(isSwitchSet("", "version",
		RTAParser.RTA_NOT_STACKABLE,
		"show current version of tool"));
	String locale =
		getParameterString("", "locale",
