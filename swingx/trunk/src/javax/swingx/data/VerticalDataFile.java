...
public class VerticalDataFile {
    ...
    private static final Logger logger =
	    Logger.getLogger(VerticalDataFile.class);

    private DateFormat dateFormat =
	    new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH);
    private VerticalDataFileFormat fileFormat =
	    VerticalDataFileFormat.TAB_SEPARATED;
	...
	for (int index = 0; index < types.length; index++) {
	    types[index] =
		    ValueType.fromClass(TypeWrapper.PRIMITIVE_WRAPPERS[0]);
	}
	while ((line = readLine(f)) != null) {
	    ...
	    for (int index = 0; index < (width < splits.length ? width
		    : splits.length); index++) {
		ValueType newType =
			ValueType.recognizeFromString(splits[index]);
	...
	try {
		...
		for (int col = 0; col < data.getColumnNumber(); col++) {
		    ...
		    if (data.getType(col).getClassObject().equals(
			    Date.class)) {
			f.writeBytes(dateFormat.format(data.getDate(row,
				col)));
		    } else {
