 ...
 * translations for multiple translations to support the multi language
...
public class MultiLanguageTranslations implements Cloneable {
    ...
    private Hashtable<String, LanguageSet> translations =
	    null;
     ...
     * @return
     */
    static public MultiLanguageTranslations from(
	    String source, String file, int line) {
	MultiLanguageTranslations hash =
		new MultiLanguageTranslations();
    ...
    public void setTranslations(
	    Hashtable<String, LanguageSet> translations) {
	if (translations == null) {
	    throw new IllegalArgumentException(
		    "translations must no be null!");
	}
	this.translations =
		(Hashtable<String, LanguageSet>) translations
			.clone();
	...
	return (Hashtable<String, LanguageSet>) translations
		.clone();
    }

    public LanguageSet getTranslations(String source) {
	return (LanguageSet) translations.get(source)
		.clone();
    }
	...
	if (!translations.containsKey(source)) {
	    translations.put(source,
		    new LanguageSet(source));
	}
    }

    public void set(String source, String language,
	    String translation) {
	set(source);
    ...
    public void addLocation(String source,
	    SourceLocation location) {
	translations.get(source).addLocation(location);
    }

    public void addLocations(String source,
	...
	Vector<SourceLocation> locations =
		translations.get(source).getLocations();
	if (locations == null) {
	    return;
	}
	for (int index = 0; index < locations.size(); index++) {
	...
	if (getClass() != obj.getClass())
	    return false;
	MultiLanguageTranslations other =
		(MultiLanguageTranslations) obj;
    ...
    public Object clone() {
	try {
	    MultiLanguageTranslations cloned =
		    (MultiLanguageTranslations) super
			    .clone();
	    cloned.translations =
		    (Hashtable<String, LanguageSet>) this.translations
			    .clone();
	    return cloned;
