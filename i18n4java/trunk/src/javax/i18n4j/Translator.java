...
/**
 * This small object was inspired by QT's approach to internationalization
 * which is much more flexible and much easier to use. This object was to be
 * created because the String object is final and a tt() function could not be
 * implemented in that way.
 * 
 * This class is the central translator for all translations. It's
 * instanciated at best as
 * 
...
public class Translator implements Serializable {
	...
	logger.info("Setting default locale to '"
		+ locale.toString() + "'");
     ...
     * sensitive instances.
     */
    private static Hashtable<String, Translator> instances =
	    new Hashtable<String, Translator>();

    /**
     * This method creates a new instance of Translator. This method should
     * only be used, if it is really necessary. getInstance() should be used
     * instead.
     * 
     * @see #getTranslator()
     * 
	...
	logger.debug("Creating instance for class '"
		+ clazz.getName() + "'");
     ...
     * method is exclusively called by getInstance to create the unique
     * instance in case it was not created before.
     * 
     * @see #getTranslator()
     * 
     * @return A reference to a newly created Translator object is returned.
     */
    private static synchronized Translator createInstance(
	    Class<?> clazz) {
    ...
    /**
     * This method returns the static instance to Translator which is unique
     * to the whole system. This is the static method to build the Singleton
     * pattern.
     * 
	...
	Translator translator =
		(Translator) instances.get(context);
	if (translator == null) {
	...
	logger.info("Set default locale to '"
		+ locale.toString() + "'");
	defaultLocale = locale;
	resetAllInstances();
    }
    ...
    static public void setAdditionalLocales(
    ...
    /**
     * This is the Hashtable for the context translations. Everything is kept
     * in there. The context is the name of the class including the package
     * name.
     */
    private Hashtable<String, SingleLanguageTranslations> translations =
	    null;
    ...
    protected void setTranslation(String source,
	    String language, String translation) {
	if (translations == null) {
	    translations =
		    new Hashtable<String, SingleLanguageTranslations>();
	}
	...
	logger
		.debug("read context translation for context '"
			+ context
			+ "' and language '"
			+ language + "'");
	...
	String resource =
		TRFile.getResource(context, language);
	logger.info("Read context language file '"
		+ resource + "'");
	InputStream is =
		getClass().getResourceAsStream(resource);
	if (is == null) {
	    logger
		    .warn("No context translation file found for '"
			    + resource + "'");
	    return;
	}
    ...
    private SingleLanguageTranslations readFromStream(
	    InputStream is) {
	try {
	    SingleLanguageTranslations translations =
		    TRFile.read(is);
	...
	SingleLanguageTranslations singleLanguageTranslations =
		translations.get(language);
	if (singleLanguageTranslations == null) {
	    return text;
	}
	String translation =
		singleLanguageTranslations.get(text);
	if (translation == null) {
	    return text;
	}
	return translation;
	...
	StringBuffer translation =
		new StringBuffer(translate(text,
			getDefaultLanguage()));
	...
	StringBuffer translation =
		new StringBuffer(new MessageFormat(
			translate(text,
				getDefaultLanguage()),
	...
	if (additionalLocales != null) {
		...
		translation.append("(").append(
			new MessageFormat(translate(text,
				locale.getLanguage()),
